...
int main(const int argc, const char* argv[])
{
    ...
    for (unsigned int i = 0, lineCount = assemblyLines.size(); i < lineCount; ++i)
    {
        ...
        else
        {
            ...
            if (line.contains('='))
            {
                instructionIndex = line.find('=');
                for (std::string assignment = line.substr(0, instructionIndex); char c : assignment)
                {
                    switch (c)
                    {
                    case 'A':
            ...
            std::string comp = line.substr(instructionIndex + 1, semicolon);
            if (comp.contains('M'))
            {
                if (comp.contains('A'))
                {
